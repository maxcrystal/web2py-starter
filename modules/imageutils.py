...
class RESIZE(object):
    ...
    def __init__(self, nx=160, ny=80, quality=100, padding=False, error_message='Error while image resize'):
    ...
    def __call__(self, value):
        ...
        try:
            ...
            if self.padding:
                ...
                background.save(s, 'PNG', quality=self.quality)
            else:
                img.save(s, 'PNG', quality=self.quality)
            s.seek(0)
            value.file = s
        except Exception as e:
            return value, f'{self.error_message}: {str(e)}'
        else:
            return value, None
